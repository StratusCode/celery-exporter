...
def test_integration(broker, celery_app, threaded_exporter, hostname):
    ...
    assert (
        f'celery_task_sent_total{{hostname="{hostname}",name="test_cli.succeed",queue_name="celery"}} 2.0'
        in res.text
    )
    assert (
        f'celery_task_sent_total{{hostname="{hostname}",name="test_cli.fail",queue_name="celery"}} 1.0'
        in res.text
    )
    assert (
        f'celery_task_received_total{{hostname="{hostname}",name="test_cli.succeed",queue_name="celery"}} 2.0'
        in res.text
    )
    assert (
        f'celery_task_received_total{{hostname="{hostname}",name="test_cli.fail",queue_name="celery"}} 1.0'
        in res.text
    )
    assert (
        f'celery_task_started_total{{hostname="{hostname}",name="test_cli.succeed",queue_name="celery"}} 2.0'
        in res.text
    )
    assert (
        f'celery_task_started_total{{hostname="{hostname}",name="test_cli.fail",queue_name="celery"}} 1.0'
        in res.text
    )
    assert (
        f'celery_task_succeeded_total{{hostname="{hostname}",name="test_cli.succeed",queue_name="celery"}} 2.0'
        in res.text
    )
    assert (
        f'celery_task_failed_total{{exception="HTTPError",hostname="{hostname}",name="test_cli.fail",queue_name="celery"}} 1.0'
        in res.text
    )
    assert (
        f'celery_task_runtime_count{{hostname="{hostname}",name="test_cli.succeed",queue_name="celery"}} 2.0'
        in res.text
    )
